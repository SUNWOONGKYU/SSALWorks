...
5. [PROJECT SAL GRID 생성 프로세스](#5-project-ssal-grid-생성-프로세스)
...
```
파일 위치:
G:\내 드라이브\Project_SSAL_Grid\template\supabase\schema.sql
...
2. `project_ssal_grid_tasks` 테이블이 보이면 성공!
...
```
파일 위치:
G:\내 드라이브\Project_SSAL_Grid\STANDARD_PROJECT_SAL_GRID_FINAL.sql
...
☑ "Success" 메시지 확인
☑ Table Editor에 project_ssal_grid_tasks 테이블 생성됨
☑ 테이블이 비어있음 (0 rows)
...
**위치:**
```
G:\내 드라이브\Project_SSAL_Grid\template\viewer\
├── viewer.html (메인 파일)
...
```bash
# 터미널 또는 명령 프롬프트에서:
cd "G:\내 드라이브\Project_SSAL_Grid\template\viewer"
...
**기본 URL:**
```
https://[your-project].supabase.co/rest/v1/project_ssal_grid_tasks
```
...
curl "https://[project].supabase.co/rest/v1/project_ssal_grid_tasks?stage=eq.4&area=eq.F"

# 진행률 50% 이상
curl "https://[project].supabase.co/rest/v1/project_ssal_grid_tasks?progress=gte.50"

# Status가 Pending 또는 In Progress
curl "https://[project].supabase.co/rest/v1/project_ssal_grid_tasks?status=in.(Pending,In%20Progress)"
```

**정렬:**
```bash
# Stage 오름차순
curl "https://[project].supabase.co/rest/v1/project_ssal_grid_tasks?order=stage.asc"

# 진행률 내림차순
curl "https://[project].supabase.co/rest/v1/project_ssal_grid_tasks?order=progress.desc"
```

**제한:**
```bash
# 처음 10개만
curl "https://[project].supabase.co/rest/v1/project_ssal_grid_tasks?limit=10"

# 5개 건너뛰고 10개
curl "https://[project].supabase.co/rest/v1/project_ssal_grid_tasks?offset=5&limit=10"
```
...
**화면에 표시:**
```
ERROR: relation "project_ssal_grid_tasks" already exists
```
...
**화면에 표시:**
```
ERROR: duplicate key value violates unique constraint "project_ssal_grid_tasks_pkey"
...
**화면에 표시:**
```
ERROR: permission denied for table project_ssal_grid_tasks
```
...
```sql
-- RLS 일시 비활성화 (개발용)
ALTER TABLE project_ssal_grid_tasks DISABLE ROW LEVEL SECURITY;

-- 데이터 입력 후 다시 활성화
ALTER TABLE project_ssal_grid_tasks ENABLE ROW LEVEL SECURITY;
```
...
```sql
-- 기존 정책 삭제
DROP POLICY IF EXISTS "Allow public read access" ON project_ssal_grid_tasks;
DROP POLICY IF EXISTS "Allow public write access" ON project_ssal_grid_tasks;
...
1. Table Editor → project_ssal_grid_tasks 테이블 클릭
...
```sql
-- 자주 검색하는 컬럼에 인덱스
CREATE INDEX IF NOT EXISTS idx_stage ON project_ssal_grid_tasks(stage);
CREATE INDEX IF NOT EXISTS idx_area ON project_ssal_grid_tasks(area);
CREATE INDEX IF NOT EXISTS idx_status ON project_ssal_grid_tasks(status);
CREATE INDEX IF NOT EXISTS idx_priority ON project_ssal_grid_tasks(priority);
```
