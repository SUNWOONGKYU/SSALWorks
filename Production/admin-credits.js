...
async function loadBalances() {
    try {
        let query = supabase
            .from('users')
            .select('id, name, email, credit_balance, created_at', { count: 'exact' })
    ...
    tbody.innerHTML = transactions.map(tx => {
        ...
        return `
            <tr>
                ...
                <td>${tx.users?.name || tx.users?.email || '-'}</td>
                <td>${typeBadge}</td>
...
async function searchUsers(searchTerm) {
    ...
    try {
        ...
        suggestions.innerHTML = users.map(user => `
            <div class="suggestion-item" onclick="selectUser('${user.id}', '${user.name || user.email}', ${user.credit_balance})">
                <div>
