...
// Order Sheet 자동 발행 엔드포인트 (프로토타입용)
// /save-to-inbox는 /save의 별칭으로, MD 파일을 Inbox에 저장
app.post('/save-to-inbox', (req, res) => {
    try {
        ...
        // 파일 경로 (항상 Inbox에 저장)
        const filePath = path.join(INBOX_DIR, finalFilename);
        ...
        res.json({
            ...
            filename: finalFilename,
            path: filePath,
            message: `Order Sheet가 Inbox에 저장되었습니다.`
        });
...
app.get('/ordersheet-templates', (req, res) => {
    try {
        ...
        if (!fs.existsSync(newTemplateDir)) {
            return res.status(404).json({
                success: false,
                error: '템플릿 디렉토리를 찾을 수 없습니다.'
            });
        }

        const templates = {};

        // 모든 하위 폴더 스캔 (P1_, P2_, P3_, S1~S5, 특별단계)
        const folders = fs.readdirSync(newTemplateDir).filter(f =>
            fs.statSync(path.join(newTemplateDir, f)).isDirectory()
        );

        folders.forEach(folder => {
            const folderPath = path.join(newTemplateDir, folder);
            ...
            mdFiles.forEach(mdFile => {
                ...
                // 파일명에서 키 생성 (예: P1-1_Vision_Mission.md → P1-1)
                const key = mdFile.replace('.md', '');

                // 제목 추출 (첫 번째 # 라인)
                const titleMatch = content.match(/^#\s+(.+)$/m);
                const title = titleMatch ? titleMatch[1].replace('Order Sheet - ', '') : key;
