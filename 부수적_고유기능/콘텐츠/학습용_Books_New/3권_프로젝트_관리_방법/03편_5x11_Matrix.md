...
결제 시스템이 핵심이다. PG사(Payment Gateway, 결제 대행 업체인 토스 페이먼트 등)와 연동해서 실제 결제가 되게 만든다. 관리자 대시보드를 만들어서 운영자가 서비스를 관리할 수 있게 한다.
...
외부 서비스 API를 연동한다. OpenAI API, 토스 페이먼트 API, Resend 이메일 API 같은 것들이다. 우리가 만든 것이 아니라 다른 회사의 서비스를 가져다 쓰는 것이다.
