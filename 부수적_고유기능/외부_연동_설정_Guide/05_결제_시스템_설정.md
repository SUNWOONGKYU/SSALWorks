# 04. 결제 시스템 설정 가이드

> 토스 페이먼트 연동 설정

**작성일**: 2025-12-17
**버전**: 1.0
**상태**: 📝 본개발 단계에서 상세 작성 예정

---

## 목차

1. [개요](#1-개요)
2. [전체 구조도](#2-전체-구조도)
3. [Phase 1: 가맹점 등록](#3-phase-1-가맹점-등록)
4. [Phase 2: API 키 발급](#4-phase-2-api-키-발급)
5. [Phase 3: 환경변수 설정](#5-phase-3-환경변수-설정)
6. [Phase 4: 빌링키 발급 (정기결제)](#6-phase-4-빌링키-발급-정기결제)
7. [Phase 5: 웹훅 설정](#7-phase-5-웹훅-설정)
8. [Phase 6: PG 이용약관 동의 UI](#8-phase-6-pg-이용약관-동의-ui)
9. [체크리스트](#9-체크리스트)

---

## 1. 개요

### 목표
- 토스 페이먼트로 구독 결제 시스템 구현
- 빌링키 발급 및 정기결제 자동화
- 결제 웹훅으로 상태 관리

### 관련 서비스
| 서비스 | 역할 |
|--------|------|
| **토스 페이먼트** | PG사, 결제 처리 |
| **Supabase** | 구독 상태 저장 |
| **Vercel** | 웹훅 엔드포인트 |

### ⚠️ 사전 준비 (중요!)

토스 페이먼트 연동 전 필수 확인:
- **가맹점 심사**: 1-2주 소요
- **사업자등록증** 필요
- **서비스 URL** 확정 필요

---

## 2. 전체 구조도

```
사용자
    ↓ 구독 결제 요청
SSALWorks 앱
    ↓ 카드 정보 입력 (토스 SDK)
토스 페이먼트
    ↓ 빌링키 발급
SSALWorks API
    ↓ 빌링키 저장
Supabase (user_payment_methods)
    ↓ 정기결제 실행
토스 페이먼트
    ↓ 결제 결과 웹훅
SSALWorks API (/api/webhook/toss)
    ↓ 구독 상태 업데이트
Supabase (users.subscription_status)
```

---

## 3. Phase 1: 가맹점 등록

### 3.1 토스 페이먼트 개발자센터 가입
```
https://developers.tosspayments.com/
→ 회원가입
→ 개발자센터 로그인
```

### 3.2 가맹점 신청

**필요 서류**:
- 사업자등록증
- 대표자 신분증
- 통장 사본
- 서비스 URL

**심사 기간**: 약 1-2주

### 3.3 테스트 모드

심사 완료 전에도 **테스트 API 키**로 개발 가능:
```
테스트 클라이언트 키: test_ck_xxx
테스트 시크릿 키: test_sk_xxx
```

---

## 4. Phase 2: API 키 발급

### 4.1 API 키 확인

```
토스 페이먼트 개발자센터 → 내 개발정보 → API 키

테스트 키:
- 클라이언트 키: test_ck_xxxxxxxxxxxxxxxx
- 시크릿 키: test_sk_xxxxxxxxxxxxxxxx

라이브 키 (심사 완료 후):
- 클라이언트 키: live_ck_xxxxxxxxxxxxxxxx
- 시크릿 키: live_sk_xxxxxxxxxxxxxxxx
```

### 4.2 키 용도

| 키 종류 | 용도 | 노출 가능 |
|---------|------|----------|
| 클라이언트 키 | 프론트엔드 SDK | ✅ 가능 |
| 시크릿 키 | 서버 API 호출 | ❌ 절대 금지 |

---

## 5. Phase 3: 환경변수 설정

### 5.1 로컬 개발 (.env.local)

```bash
# 토스 페이먼트 (테스트)
TOSS_CLIENT_KEY=test_ck_xxxxxxxxxxxxxxxx
TOSS_SECRET_KEY=test_sk_xxxxxxxxxxxxxxxx

# 토스 페이먼트 (프로덕션) - 심사 완료 후
# TOSS_CLIENT_KEY=live_ck_xxxxxxxxxxxxxxxx
# TOSS_SECRET_KEY=live_sk_xxxxxxxxxxxxxxxx
```

### 5.2 Vercel 환경변수

```
Vercel Dashboard → Settings → Environment Variables

| Key | Value | Environment |
|-----|-------|-------------|
| TOSS_CLIENT_KEY | test_ck_xxx... | Preview, Development |
| TOSS_SECRET_KEY | test_sk_xxx... | Preview, Development |
| TOSS_CLIENT_KEY | live_ck_xxx... | Production |
| TOSS_SECRET_KEY | live_sk_xxx... | Production |
```

---

## 6. Phase 4: 빌링키 발급 (정기결제)

### 6.1 토스 SDK 로드

```html
<script src="https://js.tosspayments.com/v1/payment"></script>
```

### 6.2 빌링키 발급 요청

```javascript
const tossPayments = TossPayments('test_ck_xxxxxxxxxxxxxxxx');

// 빌링키 발급 (카드 등록)
async function requestBillingKey() {
  await tossPayments.requestBillingAuth('카드', {
    customerKey: 'user_uuid_here',  // 사용자 고유 ID
    successUrl: 'https://www.ssalworks.ai.kr/api/payment/billing/success',
    failUrl: 'https://www.ssalworks.ai.kr/api/payment/billing/fail',
  });
}
```

### 6.3 빌링키 저장 (서버)

```javascript
// /api/payment/billing/success
export async function GET(request) {
  const { authKey, customerKey } = request.query;

  // 토스 API로 빌링키 발급 확정
  const response = await fetch('https://api.tosspayments.com/v1/billing/authorizations/issue', {
    method: 'POST',
    headers: {
      'Authorization': `Basic ${Buffer.from(TOSS_SECRET_KEY + ':').toString('base64')}`,
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({
      authKey,
      customerKey,
    }),
  });

  const { billingKey, card } = await response.json();

  // Supabase에 빌링키 저장
  await supabase.from('user_payment_methods').insert({
    user_id: customerKey,
    billing_key: billingKey,
    card_company: card.company,
    card_number_masked: card.number,  // 마스킹된 카드번호
  });
}
```

### 6.4 정기결제 실행

```javascript
// 매월 정기결제 실행 (Cron Job 또는 Vercel Cron)
async function executeSubscriptionPayment(userId, billingKey, amount) {
  const response = await fetch('https://api.tosspayments.com/v1/billing/' + billingKey, {
    method: 'POST',
    headers: {
      'Authorization': `Basic ${Buffer.from(TOSS_SECRET_KEY + ':').toString('base64')}`,
      'Content-Type': 'application/json',
    },
    body: JSON.stringify({
      customerKey: userId,
      amount: amount,
      orderId: `order_${Date.now()}`,
      orderName: 'SSALWorks 월 구독',
    }),
  });

  return response.json();
}
```

---

## 7. Phase 5: 웹훅 설정

### 7.1 웹훅 URL 등록

```
토스 페이먼트 개발자센터 → 웹훅 설정

웹훅 URL: https://www.ssalworks.ai.kr/api/webhook/toss
```

### 7.2 웹훅 엔드포인트 구현

```javascript
// /api/webhook/toss
export async function POST(request) {
  const payload = await request.json();

  // 웹훅 시그니처 검증 (보안)
  const signature = request.headers.get('Toss-Signature');
  // ... 시그니처 검증 로직

  switch (payload.eventType) {
    case 'PAYMENT_STATUS_CHANGED':
      if (payload.status === 'DONE') {
        // 결제 성공 → 구독 상태 업데이트
        await supabase.from('users').update({
          subscription_status: 'active',
          subscription_start_date: new Date(),
        }).eq('user_id', payload.customerKey);
      }
      break;

    case 'BILLING_KEY_DELETED':
      // 빌링키 삭제 → 결제 수단 삭제
      await supabase.from('user_payment_methods')
        .delete()
        .eq('billing_key', payload.billingKey);
      break;
  }

  return new Response('OK');
}
```

---

## 8. Phase 6: PG 이용약관 동의 UI

### ⚠️ 법적 필수 요소!

결제 진행 전 다음 약관 동의 필수:

### 8.1 필수 약관

| 약관 | 법적 근거 |
|------|----------|
| 전자금융거래 이용약관 | 전자금융거래법 |
| 개인정보 제3자 제공 동의 | 개인정보보호법 |
| 결제대행 서비스 이용약관 | PG사 요구 |

### 8.2 UI 구현 예시

```html
<div class="payment-terms">
  <h3>결제 약관 동의</h3>

  <label>
    <input type="checkbox" id="terms-all" onchange="toggleAllTerms(this)">
    <strong>전체 동의</strong>
  </label>

  <label>
    <input type="checkbox" class="term-checkbox" required>
    [필수] 전자금융거래 이용약관
    <a href="#" onclick="showTermsPopup('efin')">보기</a>
  </label>

  <label>
    <input type="checkbox" class="term-checkbox" required>
    [필수] 개인정보 제3자 제공 동의 (토스페이먼트)
    <a href="#" onclick="showTermsPopup('privacy')">보기</a>
  </label>

  <label>
    <input type="checkbox" class="term-checkbox" required>
    [필수] 결제대행 서비스 이용약관
    <a href="#" onclick="showTermsPopup('pg')">보기</a>
  </label>
</div>

<button onclick="proceedPayment()" id="pay-button" disabled>
  결제하기
</button>
```

### 8.3 동의 여부 DB 저장

```javascript
// 결제 진행 전 동의 여부 저장
await supabase.from('payment_consents').insert({
  user_id: userId,
  efin_terms_agreed: true,
  privacy_third_party_agreed: true,
  pg_terms_agreed: true,
  agreed_at: new Date(),
});
```

---

## 9. 체크리스트

### Phase 1: 가맹점 등록
- [ ] 토스 페이먼트 개발자센터 가입
- [ ] 가맹점 심사 신청 (1-2주 소요)
- [ ] 테스트 API 키로 개발 시작

### Phase 2: API 키
- [ ] 테스트 클라이언트 키 확인
- [ ] 테스트 시크릿 키 확인
- [ ] (심사 완료 후) 라이브 키 확인

### Phase 3: 환경변수
- [ ] 로컬 .env.local 설정
- [ ] Vercel 환경변수 설정 (테스트/프로덕션 분리)

### Phase 4: 빌링키 발급
- [ ] 토스 SDK 연동
- [ ] 빌링키 발급 API 구현
- [ ] 빌링키 저장 테이블 (user_payment_methods)

### Phase 5: 웹훅
- [ ] 웹훅 URL 등록
- [ ] 웹훅 엔드포인트 구현
- [ ] 결제 상태 업데이트 로직

### Phase 6: 약관 동의
- [ ] 전자금융거래 이용약관 동의 UI
- [ ] 개인정보 제3자 제공 동의 UI
- [ ] 동의 여부 DB 저장

### 테스트
- [ ] 테스트 카드로 결제 테스트
- [ ] 정기결제 테스트
- [ ] 웹훅 수신 테스트
- [ ] 결제 취소/환불 테스트

---

## 참고 문서

- [토스 페이먼트 개발자 문서](https://docs.tosspayments.com/)
- [빌링키 발급 가이드](https://docs.tosspayments.com/guides/billing)
- [결제 웹훅 가이드](https://docs.tosspayments.com/guides/webhook)
- [테스트 카드 정보](https://docs.tosspayments.com/reference/test-card)

---

## ⚠️ 본개발 단계 TODO

이 문서는 기본 구조만 정리되어 있습니다.
본개발 단계에서 다음 항목을 상세 작성해야 합니다:

- [ ] 실제 빌링키 발급 테스트 결과
- [ ] 정기결제 Cron Job 설정 방법
- [ ] 결제 실패 시 재시도 로직
- [ ] 환불 처리 플로우
- [ ] 에러 핸들링 상세

---

**Last Updated**: 2025-12-17
