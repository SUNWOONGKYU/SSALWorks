...
#### CLAUDE.md — 개념과 도메인 지식 정의
> **셀**: S1CO | **분산 배치**
> **관련**: CLAUDE.md 전체 가이드는 → S1ST 참조

- CLAUDE.md에 프로젝트의 비즈니스 도메인과 핵심 개념을 기술
- 예: "이 프로젝트는 SaaS 구독 관리 서비스이며, 핵심 엔터티는 User, Subscription, Plan이다"
- AI가 프로젝트 맥락을 이해하고 적절한 용어와 패턴을 사용하게 됨
- 도메인 특화 용어 사전(glossary)을 포함하면 AI의 정확도 향상

#### Plugin Marketplace — 플러그인 생태계 개념
> **셀**: S1CO | **분산 배치**
> **관련**: 플러그인 생성/배포 실무는 → WF 영역 참조

- Plugin Marketplace: 커뮤니티가 만든 플러그인을 발견하고 설치할 수 있는 생태계
- 플러그인 = MCP 서버 + 커스텀 슬래시 명령어 + 에이전트를 하나의 패키지로 묶은 것
- 마켓플레이스를 통해 팀 간 도구 공유, 재사용 가능한 자동화 패턴 유통
- 개념적으로 npm/VS Code Extension Marketplace와 유사한 배포 구조

#### Multi-Agent vs Multi-Role 시스템 개념
> **셀**: S1CO | **분산 배치**
> **참고**: `부수적_고유기능/콘텐츠/실전_Tips/도구_활용/Multi-Role_AI_시스템과_Multi-Agent_시스템_비교.md`

- Multi-Role AI: 하나의 AI가 역할을 전환하며 작업 (예: Claude가 코드 리뷰어 → 테스터 → 문서 작성자)
- Multi-Agent: 여러 독립 AI 인스턴스가 각자 전문 역할 수행 (서브에이전트, Agent Teams)
- 오케스트레이터 개념: Main Agent가 서브에이전트들을 조율하는 지휘 구조
- 협업 패턴: 계층형(Hierarchical), 파이프라인(Pipeline), P2P(Peer-to-Peer)
...
#### Remote Control (원격 사용) 설정 구조
> **셀**: S1ST | **분산 배치**
> **관련**: Claude Code Remote — 원격 환경에서 Claude Code를 실행하기 위한 구조적 설정

- SSH 기반 원격 접속: `claude --remote <host>` 형태로 원격 머신에서 Claude Code 실행
- 서버 측 설정: Node.js + Claude Code 설치, SSH 접속 허용 구성
- 포트 포워딩: 로컬 ↔ 원격 간 통신 채널 구성
- 원격 워크스페이스 지정: 원격 서버의 프로젝트 디렉토리 경로 설정

#### Memory (메모리) 설정 구조
> **셀**: S1ST | **분산 배치**
> **관련**: Memory는 세션 간 지식을 유지하는 구조적 저장소

- 메모리 저장 위치: `~/.claude/projects/<project>/memory/` 디렉토리
- `MEMORY.md`: 항상 컨텍스트에 로드되는 핵심 메모리 파일 (200줄 제한)
- 토픽별 분리: `debugging.md`, `patterns.md` 등 세부 주제 파일로 분산 저장
- MEMORY.md에서 세부 파일로 링크하여 참조 구조 형성

#### .claudeignore — 파일 제외 구조
> **셀**: S1ST | **분산 배치**
> **관련**: .gitignore와 유사한 구조로, AI가 접근하지 않을 파일/폴더를 지정

- 파일 위치: 프로젝트 루트에 `.claudeignore` 파일 생성
- 문법: .gitignore와 동일 (glob 패턴 사용)
- 용도: 대용량 바이너리, 빌드 산출물, 민감 정보 파일 등을 AI 컨텍스트에서 제외
- 예: `node_modules/`, `*.env`, `dist/`, `*.log` 등 패턴 지정

#### Doctor Command — 환경 진단 도구
> **셀**: S1ST | **분산 배치**
> **관련**: 설치 후 환경 문제 발생 시 진단용 명령어

- 사용법: `claude doctor` 명령으로 현재 환경 상태 진단
- 점검 항목: Node.js 버전, API 키 유효성, 네트워크 연결, 설정 파일 무결성
- 문제 발견 시 자동으로 해결 방법 안내
- 설치 직후 또는 오류 발생 시 가장 먼저 실행할 명령어

#### Status Line — 상태 표시줄 커스터마이징
> **셀**: S1ST | **분산 배치**

- Claude Code 하단 상태 표시줄을 사용자 정의 가능
- 현재 모델, 토큰 사용량, 세션 비용 등 실시간 표시
- 설정: `~/.claude/settings.json`에서 `statusLine` 필드 구성

#### Keybindings — 단축키 설정
> **셀**: S1ST | **분산 배치**

- 키보드 단축키를 사용자 환경에 맞게 재매핑 가능
- VS Code/JetBrains 통합 시 IDE 단축키와 충돌 방지 설정
- 설정 파일에서 키 바인딩 커스터마이징

#### Output Styles — 출력 스타일 설정
> **셀**: S1ST | **분산 배치**

- Claude Code 응답의 출력 형식을 설정: `--output-format` 플래그
- 지원 형식: text, json, stream-json 등
- 파이프라인 연동 시 JSON 출력으로 프로그래밍적 처리 가능

#### DevContainer — 개발 컨테이너 환경
> **셀**: S1ST | **분산 배치**

- Docker 기반 개발 컨테이너에서 Claude Code 실행 환경 구성
- `.devcontainer/devcontainer.json`에서 Claude Code 포함 설정
- GitHub Codespaces, VS Code Dev Containers와 통합
- 팀 전체가 동일한 개발 환경을 공유할 수 있는 구조

#### LLM Gateway — 엔터프라이즈 LLM 라우팅
> **셀**: S1ST | **분산 배치**

- 기업 환경에서 LLM 요청을 프록시/게이트웨이를 통해 라우팅
- API 키 중앙 관리, 요청 로깅, 비용 할당 등 엔터프라이즈 기능
- 지원: AWS Bedrock, Google Vertex AI, Microsoft Foundry, 커스텀 게이트웨이

#### Server Managed Settings — 서버 관리 설정
> **셀**: S1ST | **분산 배치**

- 조직 관리자가 서버 측에서 Claude Code 설정을 중앙 관리 (퍼블릭 베타)
- 허용 모델, 권한 정책, MCP 서버 목록 등을 서버에서 일괄 배포
- 개별 사용자의 설정을 조직 정책으로 오버라이드 가능

#### Network Config — 엔터프라이즈 네트워크 설정
> **셀**: S1ST | **분산 배치**

- 기업 프록시, 방화벽 환경에서 Claude Code 네트워크 구성
- HTTP/HTTPS 프록시 설정, SSL 인증서, 커스텀 CA 번들 지정
- 에어갭(air-gapped) 환경에서의 오프라인 설정 가이드
...
#### CLAUDE.md — 개발 프로세스 및 실행 순서 정의
> **셀**: S1WF | **분산 배치**
> **관련**: CLAUDE.md 전체 가이드는 → S1ST 참조

- CLAUDE.md에 개발 워크플로우와 실행 프로세스를 명시
- 예: "파일 저장 시 Stage 폴더에 먼저 저장 → Pre-commit Hook으로 루트에 자동 복사"
- 커밋 메시지 규칙, 브랜치 전략, 배포 프로세스 등 작업 흐름 정의
- AI가 프로세스를 준수하며 작업하도록 강제하는 핵심 수단

#### Headless 모드 — CI/CD 자동화 워크플로우
> **셀**: S1WF | **분산 배치**
> **관련**: Headless 고급 아키텍처는 → S3WF [09] Hooks 시스템 참조

- Headless 모드: 대화형 터미널 없이 Claude Code를 자동으로 실행하는 방식
- 기본 사용: `claude --headless -p "작업 지시"` (비대화형 실행)
- CI/CD 파이프라인에서 코드 리뷰, 테스트, 빌드 자동화에 활용
- GitHub Actions, Jenkins 등 자동화 도구와 연동하여 무인 실행 가능

#### Extended Thinking — 확장 사고 모드
> **셀**: S1WF | **분산 배치**
> **관련**: 복잡한 추론이 필요한 작업에서 AI의 사고 과정을 확장하는 기능

- Extended Thinking: AI가 답변 전에 더 깊이 사고하는 모드
- 복잡한 코드 아키텍처 설계, 버그 분석, 리팩토링 전략 수립 시 활용
- 토큰 소비가 증가하므로 복잡한 작업에만 선택적 사용 권장
- 관련 비용 고려사항은 → CM 영역 참조

#### Vision — 이미지 분석 기능
> **셀**: S1WF | **분산 배치**
> **관련**: Claude Code가 이미지 파일을 직접 읽고 분석하는 기능

- 스크린샷 분석: UI 버그 확인, 디자인 시안 검토에 활용
- 이미지 파일 읽기: PNG, JPG 등 이미지를 Read 도구로 직접 열어 시각적 분석
- 활용 예: "이 스크린샷에서 레이아웃 문제를 찾아줘", "와이어프레임을 보고 HTML 구현해줘"
- 멀티모달 입력으로 텍스트 + 이미지를 함께 전달하여 정확한 작업 지시 가능

#### Checkpointing — 작업 복원 지점
> **셀**: S1WF | **분산 배치**

- Checkpointing: 작업 중 자동으로 복원 지점(체크포인트)을 생성
- 문제 발생 시 이전 체크포인트로 롤백하여 안전하게 복구
- 대규모 리팩토링이나 위험한 변경 시 안전망 역할
- Git 커밋과 별도로 Claude Code 자체적으로 관리하는 복원 메커니즘

#### Chrome 연동 — 브라우저 디버깅
> **셀**: S1WF | **분산 배치**

- Chrome 브라우저와 연동하여 라이브 웹 애플리케이션 디버깅
- 브라우저 콘솔 오류를 Claude Code로 전달하여 실시간 분석
- DOM 상태, 네트워크 요청, 콘솔 로그 기반 문제 해결

#### Teleport / Desktop 핸드오프 — 세션 이동
> **셀**: S1WF | **분산 배치**

- `/teleport`: 웹/iOS에서 시작한 세션을 로컬 터미널로 가져오기
- `/desktop`: 터미널 세션을 Desktop 앱으로 핸드오프하여 시각적 diff 리뷰
- 세션 간 컨텍스트가 유지되어 어디서든 이어서 작업 가능

#### GitHub Actions / GitLab CI — CI/CD 자동화
> **셀**: S1WF | **분산 배치**
> **관련**: Headless 모드와 연계하여 CI/CD 파이프라인에서 Claude Code 자동 실행

- GitHub Actions: PR 리뷰 자동화, 이슈 분류, 코드 품질 체크
- GitLab CI/CD: 파이프라인 단계에 Claude Code 통합
- `@claude` 멘션으로 GitHub/GitLab에서 직접 AI 호출 가능
- 자동 PR 생성, 테스트 실행, 릴리스 노트 작성 등

#### Interactive Mode — 대화형 모드
> **셀**: S1WF | **분산 배치**

- Interactive Mode: 기본 실행 모드, 터미널에서 대화하며 작업 진행
- Headless와 대비되는 개념 — 사용자와 실시간으로 대화하며 확인/수정
- 도구 실행 전 사용자 승인 요청, 중간 결과 확인, 방향 수정 가능

#### /status 명령어 — 전체 상태 확인
> **셀**: S1WF | **분산 배치**
> **참고**: `부수적_고유기능/콘텐츠/실전_Tips/Claude_Code_사용법/status_명령어로_모든_것_확인하기.md`

- `/status`: 현재 세션의 모든 상태를 한눈에 확인하는 명령어
- Status 탭: 버전, 세션 ID, 현재 디렉토리, 로그인 방법
- Config 탭: 설정값 확인
- Usage 탭: 토큰 사용량, 비용 통계

#### 듀얼 모드 활용법
> **셀**: S1WF | **분산 배치**
> **참고**: `부수적_고유기능/콘텐츠/실전_Tips/Claude_Code_사용법/듀얼_모드로_Claude_Code_활용하기.md`

- 듀얼 모드: Claude.ai(웹)와 Claude Code(터미널)를 동시에 활용하는 전략
- 웹에서 설계/기획 → 터미널에서 구현/실행으로 역할 분담
- 복잡한 작업을 효율적으로 분산 처리하는 실전 기법

#### 5회 반복 디버깅 기법
> **셀**: S1WF | **분산 배치**
> **참고**: `부수적_고유기능/콘텐츠/실전_Tips/Claude_Code_사용법/5회_반복_디버깅_기법.md`

- 동일 버그에 대해 최대 5회까지 다른 접근법으로 반복 시도하는 디버깅 전략
- 1~2회차: 직접 수정, 3회차: 접근법 변경, 4~5회차: 근본 원인 재분석
- 5회 내 해결 안 되면 사용자에게 상황 보고 후 방향 전환

---

### S1TO — 팀운용 (Team Operations)

#### Slack 연동 — 팀 채팅에서 AI 호출
> **셀**: S1TO | **분산 배치**

- Slack에서 `@Claude` 멘션으로 버그 리포트 전달 → PR 자동 생성
- 팀 채팅 채널에서 직접 코딩 작업을 요청하고 결과를 받는 협업 패턴
- 비개발자(PM, 디자이너)도 Slack을 통해 AI에게 작업 요청 가능
...
#### CLAUDE.md — 팀 규칙 및 에이전트 역할 정의
> **셀**: S1TO | **분산 배치**
> **관련**: CLAUDE.md 전체 가이드는 → S1ST 참조

- CLAUDE.md에 에이전트 역할 배분과 팀 협업 규칙을 정의
- 예: "Task Agent ≠ Verification Agent (작성자와 검증자 분리)"
- 서브에이전트 투입 기준, 팀 구성 원칙 등 명시
- AI가 적절한 역할 분담을 지키며 협업하도록 유도
...
#### CLAUDE.md — 코딩 규칙 및 품질 기준 정의
> **셀**: S1QC | **분산 배치**
> **관련**: CLAUDE.md 전체 가이드는 → S1ST 참조

- CLAUDE.md에 코딩 스타일, 네이밍 규칙, 보안 규칙을 정의
- 예: "kebab-case 파일명 필수", "OWASP Top 10 취약점 방지", "검증 없이 완료 보고 금지"
- 린트 규칙, 테스트 기준, 코드 리뷰 체크리스트 포함 가능
- AI가 일관된 품질 기준을 준수하며 코드를 생성하는 핵심 장치

#### Sandboxing — 보안 격리 환경
> **셀**: S1QC | **분산 배치**

- Sandboxing: Claude Code의 파일 접근과 명령 실행을 격리된 환경으로 제한
- 민감한 시스템 파일, 다른 프로젝트 디렉토리에 대한 접근 차단
- macOS: App Sandbox, Linux: 컨테이너 기반 격리
- 프로젝트 디렉토리 외부 접근 시 사용자 명시적 승인 필요

#### Zero Data Retention — 데이터 보존 없음 정책
> **셀**: S1QC | **분산 배치**

- Zero Data Retention: API 요청 데이터를 Anthropic 서버에 저장하지 않는 정책
- 엔터프라이즈/API 사용자 대상 — 학습 데이터로 사용되지 않음 보장
- 규제 산업(금융, 의료, 정부) 환경에서의 컴플라이언스 충족
- 데이터 보안이 중요한 프로젝트에서 필수 확인 사항
...
#### CLAUDE.md — 비용 및 효율 지침 정의
> **셀**: S1CM | **분산 배치**
> **관련**: CLAUDE.md 전체 가이드는 → S1ST 참조

- CLAUDE.md에 토큰 절약 및 비용 관련 지침을 명시
- 예: "대용량 파일은 전체를 읽지 말고 필요한 부분만 offset/limit으로 읽기"
- 불필요한 도구 호출 최소화, 컨텍스트 관리 전략 등 포함
- AI의 효율적 리소스 사용을 유도하는 가이드라인

#### Prompt Caching — 프롬프트 캐싱으로 비용 절감
> **셀**: S1CM | **분산 배치**
> **관련**: 반복되는 프롬프트를 캐싱하여 API 호출 비용을 줄이는 기능

- Prompt Caching: 동일한 시스템 프롬프트나 CLAUDE.md 내용을 캐시하여 재사용
- 캐시 히트 시 입력 토큰 비용 최대 90% 절감 가능
- 긴 CLAUDE.md, 큰 코드베이스 컨텍스트일수록 캐싱 효과 극대화
- 자동으로 적용되며 별도 설정 불필요

#### Max Turns — 에이전트 턴 수 제한
> **셀**: S1CM | **분산 배치**
> **관련**: 서브에이전트의 실행 턴 수를 제한하여 비용 폭주 방지

- `--max-turns` 옵션: 에이전트가 수행할 최대 API 왕복 횟수 지정
- 예: `claude --max-turns 10` (최대 10턴으로 제한)
- 서브에이전트(Task tool)에서도 `max_turns` 파라미터로 개별 제한 가능
- 비용 관리의 핵심 수단 — 무한 루프나 과도한 탐색 방지

#### Model Selection — 모델 선택 전략
> **셀**: S1CM | **분산 배치**
> **관련**: 작업 복잡도에 따라 적절한 모델을 선택하여 비용 최적화

- Opus: 복잡한 아키텍처 설계, 대규모 리팩토링 등 고난도 작업
- Sonnet: 일반 개발 작업, 코드 수정, 버그 수정 등 표준 작업
- Haiku: 간단한 질문, 빠른 검색, 단순 변환 등 경량 작업
- `--model` 플래그 또는 서브에이전트 `model` 파라미터로 작업별 모델 지정
- 비용 차이: Opus > Sonnet > Haiku (복잡도에 맞춰 선택하면 비용 절감)

#### Fast Mode — 빠른 응답 모드
> **셀**: S1CM | **분산 배치**

- `/fast` 토글로 빠른 응답 모드 전환 (동일 모델, 더 빠른 출력)
- 간단한 작업에서 응답 속도 향상 → 토큰 절약 효과
- 복잡한 추론이 필요한 작업에서는 일반 모드 사용 권장

#### Monitoring / Analytics — 사용량 모니터링
> **셀**: S1CM | **분산 배치**

- 팀 단위 사용량 추적: 모델별, 사용자별, 프로젝트별 토큰 소비 분석
- 실시간 세션 비용 표시: 현재 세션에서 사용한 비용 실시간 확인
- 조직 관리자가 사용 패턴 분석하여 비용 최적화 전략 수립
- Anthropic Console 또는 API로 사용량 데이터 조회 가능

#### AI별 웹검색 비용과 효율적 활용법
> **셀**: S1CM | **분산 배치**
> **참고**: `부수적_고유기능/콘텐츠/실전_Tips/도구_활용/AI별_웹검색_비용과_효율적_활용법.md`

- Claude Code의 WebSearch/WebFetch 도구 사용 시 추가 토큰 소비 발생
- MCP 기반 웹검색(Perplexity 등)과 내장 웹검색의 비용 비교
- 검색 빈도 최소화 전략: 캐시 활용, 검색 결과 재사용, 정확한 쿼리 작성
