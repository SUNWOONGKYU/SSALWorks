<!-- 🔄 Auto-generated from manual_template.md on 2026-02-27 -->
...
Step 2: 5×11 매트릭스 참고
  ...
  └─ 프로젝트에 맞게 조정
         ↓
Step 3: Task 선정 + SAL ID 부여 (Provisional, 가확정)
  ├─ 원칙 1: 파일 1개 = Task 1개
  ├─ 원칙 2: 유사 작업 통합
  └─ SAL ID 형식: [Stage][Area][번호] (예: S2F1)
         ↓
Step 4: Task Instruction 작성
  ├─ 작업 목표
  ├─ 구체적 지시사항
  └─ 예상 출력물
         ↓
Step 5: Verification Instruction 작성
  ├─ 검증 기준
  ├─ 체크리스트
  └─ 합격 조건
         ↓
Step 6: 22개 속성 입력 + 의존성 검증
  ...
  └─ ⚠️ 의존성 검증 수행 (아래 참조)
    ...
    ┌─────────────────────────────────────┐
    │ 의존성 검증 체크리스트              │
    │ □ 선행 Task ID < 후행 Task ID?      │
    │ □ 동일 Stage 내 순환 의존성 없음?   │
    │ □ 존재하지 않는 Task 참조 없음?     │
    └─────────────────────────────────────┘
         ↓
    위반 발견 시 → Step 3으로 돌아가 ID 수정
    통과 시 → SAL ID 확정 (Finalization)
         ↓
PROJECT SAL GRID 완성! ✅
```

**⚠️ 핵심: Provisional → Finalization 프로세스**

SAL ID는 처음 부여 시 **가확정(Provisional)** 상태입니다.
22개 속성 입력 단계에서 의존성을 검증하고, 문제가 없으면 **최종 확정(Finalization)**됩니다.

| 단계 | SAL ID 상태 | 설명 |
|------|------------|------|
| Step 3 | Provisional (가확정) | Task 선정 시 ID 부여 |
| Step 4-5 | Provisional | Instruction 작성 |
| Step 6 | **검증** | 의존성 체크 |
| 검증 통과 | **Finalization (확정)** | Grid 완성 |
| 검증 실패 | → Step 3 | ID 수정 후 재진행 |
